...
| Área | Descrição |
|------|-----------|
| Auth | Firebase Authentication (email/senha) — requer `NEXT_PUBLIC_FIREBASE_*` no ambiente; sem estas variáveis o login é desativado. Opcional: Google sign-in. |
...
Configuração centralizada em `lib/config/firebase-env.ts` (cliente) e `lib/config/firebase-admin-env.ts` (servidor, `server-only`). Ver lista completa em [`Docs/Firebase/variaveis-ambiente.md`](Firebase/variaveis-ambiente.md) e o ficheiro [`.env.example`](../.env.example) na raiz para placeholders.

Em produção, as variáveis são configuradas no painel da Vercel — ver [`DEPLOY_VERCEL_FIREBASE.md`](../DEPLOY_VERCEL_FIREBASE.md).
...
| Backend local | Mock + SecureStore | Firebase obrigatório para área autenticada; sessão em `localStorage` |
